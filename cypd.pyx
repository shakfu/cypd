...
cdef void pd_cprint(const char *s) noexcept:
    printf("p>> %s\n", s)

cdef void pd_cbang(const char *recv) noexcept:
    printf("b>> BANG %s\n", recv)

cdef void pd_cfloat(const char *recv, float x) noexcept:
    printf("f>> float %s %f\n", recv, x)

cdef void pd_csymbol(const char *recv, const char *symbol) noexcept:
    printf("s>> symbol %s %s\n", recv, symbol)

cdef void pd_clist(const char *recv, int argc, pd.t_atom *argv) noexcept:
    printf("l>> list %s %i\n", recv, argc)

cdef void pd_cmessage(const char *recv, const char *msg, int argc, pd.t_atom *argv) noexcept:
    printf("m>> msg %s %s %i\n", recv, msg, argc)

cdef void pd_cnoteon(int channel, int pitch, int velocity) noexcept:
...
cdef class Patch:
    ...
    cdef pd.t_pdinstance *get_instance(self, int index):
        ...
        """
        # libpd_get_instance is not available in the current libpd version
